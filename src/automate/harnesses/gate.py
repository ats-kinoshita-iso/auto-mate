...
class CommandGate:
    ...
    def evaluate(self, task: Task, crew: CrewResult) -> Verdict:
        ...
        The command is invoked as ``<command> <task-id> <branch> <intent>`` with
        the task's repo as working directory, so gate scripts can inspect the
        branch and judge it against what the task set out to do. It runs with
        ``check=False``: a non-zero exit is this gate's "fail" signal and must
        surface as a failed verdict (a GATED run), not as an exception (a FAILED
        run).
        """
        if not self._command:
            return Verdict(gate=self._gate, passed=True, findings=["gate disabled"])
        result = self._runner.run(
            [*shlex.split(self._command), crew.task_id, crew.branch, task.prompt],
            cwd=task.repo,
            check=False,
